...
def ImportCoprData(name, Conf):
    ...
    for i in factorsFile.readlines():
        line = i.replace("^", "**").replace("Sqrt[2]", "sp.sqrt(2)").replace(
            "Sqrt[5]", "sp.sqrt(5)").replace("\n", "")
        factors.append(eval(line))

    # Each pair is (index of polynomial within Conf, index of factor within factors)
    factorPairs = []
...
def Get2DCoprCandidates(orbitType, copr, sharedPlanes, group,):
    ...
    for i in range(len(factors)):
        ...
        for plane in planes:
            sFacetings += FindFacetings(orbitSize,
                                        group, [0]+list(plane), minCycleLength=4)
        totalFacetingCandidates += [(s, cycle) for cycle in sFacetings]

    return totalFacetingCandidates


def ImportCopr(name): return [eval(i)
                              for i in open("data/"+name+"Copr.txt").readlines()]
...
# Take the set of critical pairs (found in the previous function)
# and exports it into a format easier to parse with Mathematica.
def ExportCriticalPairs(name, pairs, coprList):
    ...
    for poly in coprList:  # Reformat polynomials into Mathematica's format.
        f.write(str(poly).replace("**", "^").replace("sqrt(2)",
                "Sqrt[2]").replace("sqrt(5)", "Sqrt[5]").replace(" ", ""))
        f.write("\n")
    f.close()

    for pair in pairs:
        # Add 1 because Mathematica is a 1-indexing language.
        g.write(str(pair[0]+1))
        g.write(" ")
        g.write(str(pair[1]+1))
        g.write("\n")
    g.close()

# Imports the approximate a- and b- coordinates of intersections
# that Mathematica has found to exist. From there we need to determine
# which combinations of a- and b- values actually lead to a critical
# orbit which needs to be checked.
def ImportPairData(name):
    f = open("data/"+name+"RootData.txt")

    # For information about how this dictionary is structured, see enumerate2D-D.py.
    pairDataDictA = dict()
    pairDataDictB = dict()
    ...
    for line in f.readlines():
        ...
        # Pair of polynomials being analyzed for intersections.
        pair = (int(split[0]), int(split[1]))
        var = split[2]  # Variable for which a root has been found.
        # The actual approximate value of the root.
        root = float(split[3].split("`")[0])
        # The known precision of the value given.
        precision = float(split[3].split("`")[1])
...
def AreClose(n1, n2):
    uncertainty = 10**(len(str(int(n1[0])))-int(n1[1])) + \
        10**(len(str(int(n2[0])))-int(n2[1]))
...
def GetConnectedComponent(adjDict, startNode):
    ...
    # List of points that still need to be searched for additional graph connections
...
# Process pair data to find which pairs corresponds to each root of a given parameter (a or b)
def GetParameterValues(pairData):
    ...
    for pair in pairData[0].keys():
        ...
        for root in roots:
            aValues[root] = aValues.setdefault(
                root, []) + [pair]  # Add pair to this a value
    for pair in pairData[1].keys():
        roots = pairData[1][pair]
        for root in roots:
            bValues[root] = bValues.setdefault(
                root, []) + [pair]  # Add pair to this b value

    return aValues, bValues

# Take a (orbit type)Intersections.txt file and convert it into a Python dictionary with the same data.
def ImportIntersectionData(name):
    ...
    for pair, intersections in data:
        for intersection in intersections:
            intersectionDict[tuple(intersection)] = intersectionDict.setdefault(
                tuple(intersection), []) + [pair]

    return intersectionDict

# Import comprime polynomial faceting data, shared plane faceting data, and individual coprime polynomials as a Python object
def ImportFacetingData(name):
    polynomialsFile = open("data/"+name+"CoprList.txt")
    coprFile = open("data/"+name+"Copr.txt").readlines()
    copr = eval(coprFile[0])  # Coprime polynomial pair dat
    sharedPlanes = eval(coprFile[1])

    polynomials = []
    for i in polynomialsFile.readlines():
        # Convert from Mathematica to Python syntax
        line = i.replace("^", "**").replace("Sqrt[2]", "sp.sqrt(2)").replace(
            "Sqrt[5]", "sp.sqrt(5)").replace("\n", "")
        polynomials.append(eval(line)) # Coprime polynomial list

    return polynomials, copr, sharedPlanes

# Determine faceting data for the various possible intersections.
def DetermineIntersectionData(intersections, poly, copr, sharedPlanes):
    intersectionData = dict([])
    keys = list(intersections.keys())
    for n in range(len(keys)):
        k = keys[n] # k is our particular intersection

        pairs = intersections[k]
        curves = set(sum(pairs, [])) # this is the set of critical curves which pass through the intersection point

        mergedPlanes = [] # this is the set of extra planes (in addition to the shared planes) which become coplanar at the intersection.
        for i in curves:
            mergedPlanes = MergePlanes(mergedPlanes + copr[poly[i-1]])

        intersectionData[k] = SelectiveMergePlanes(
            mergedPlanes, sharedPlanes, includeOtherPlanes=True)
...
# Abstractly facet the different possible intersections
def Get2DOrbitTypeFacetings(orbitType, intersectionData, group):
    totalFacetings = []

    # The intersection is at (A,B) where A is the i-th a-value and B is the j-th 
    # b-value in their respective atlases of roots.
    for i, j in intersectionData.keys():
        facetings = []
        planes = intersectionData[(i, j)]
        for p in planes:
            facetings += FindFacetings(len(orbitType),
                                       group, [0]+list(p), minCycleLength=4)

        if len(facetings) > 0: # If we find no facetings skip the intersection
    ...
    for root in totalFacetings:
        faces = root[2] # List of faces which lead to a noble polyhedron under the symmetry group

        totalRootFacetings = []
        checkedFaces = []
        for f in faces:
            # Skip this face if it's equivalent to some other face we've already added to our final list
...
# Import atlases of roots for a given orbit type.
def ImportAtlasData(name):
    ...
    for i in lines[1][1:-2].split(","):
        aPolynomials.append(eval(i[:-4].replace("^", "**").replace("Sqrt[2]", "sp.sqrt(2)").replace(
            "Sqrt[5]", "sp.sqrt(5)").replace("Sqrt[10]", "sp.sqrt(10)")))

    #Convert from Mathematica to Python syntax
    bPolynomials = []
    for i in lines[4][1:-2].split(","):
        bPolynomials.append(eval(i[:-4].replace("^", "**").replace("Sqrt[2]", "sp.sqrt(2)").replace(
            "Sqrt[5]", "sp.sqrt(5)").replace("Sqrt[10]", "sp.sqrt(10)")))

    #[approximate a-values, exact minimal polynomials for a, approximate b-values, exact minimal polynomials for b]
...
# Export the facetings we previously found abstractly as OFF files.
def Export2DOrbitTypeFacetings(orbitType, atlas, facetings, group, directory, name):
    ...
    for vertexSet in facetings:
        ...
        # Obtain approximate vertex coordinates to use in our OFF file.
        vertices = DeepEval2D(orbitType, aValue, bValue)

        for i in range(len(faces)):
            face = faces[i]
            print("Exporting faceting", face, "of",
                  name, "at", [aValue, bValue])

            fileName = name+"."+str(aValue)+"."+str(bValue)+"."+str(i)

            ExportToOFF(vertices, face, group, directory, fileName)
            WriteSummary(fileName, [aPoly, bPoly], secondParameter=True) # aPoly and bPoly are minimal polynomials
